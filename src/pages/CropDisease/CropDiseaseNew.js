...
// Custom formatter for bot messages with ** styling
  ...
  return (
    <div className="bot-response-text">
      {parts.map((part, idx) => {
        if (part.match(/^\*\*.*\*\*$/)) {
          return (
            <strong key={idx} style={{ color: '#2d6a4f', fontWeight: '700', display: 'block', marginTop: '0.8rem', marginBottom: '0.4rem', fontSize: '1.05em' }}>
              {part.replace(/\*\*/g, '')}
            </strong>
          );
        }
        ...
        return (
          <div key={idx} style={{ margin: '0.2rem 0' }}>
            {part.split('\n').map((line, lineIdx) => {
              ...
              if (line.match(/^\d+\./)) {
                return (
                  <div key={lineIdx} style={{ marginLeft: '1rem', marginBottom: '0.5rem', display: 'flex', gap: '0.5rem' }}>
              ...
              if (line.match(/^-/)) {
                return (
                  <div key={lineIdx} style={{ marginLeft: '1rem', marginBottom: '0.4rem', display: 'flex', gap: '0.5rem' }}>
              ...
              return (
                <p key={lineIdx} style={{ margin: '0.3rem 0', lineHeight: '1.5' }}>
                  {line}
                </p>
              );
            })}
              ...
              {messages.length === 0 ? (
                ...
                messages.map((message, idx) => (
                  <div key={idx} style={{
                    padding: '1rem',
                    borderRadius: '12px',
                    backgroundColor: message.sender === 'user' ? '#40916c' : '#f8f9fa',
                    color: message.sender === 'user' ? 'white' : '#1b3a1b',
                    border: message.sender === 'bot' ? '2px solid #40916c' : 'none',
                    ...
                    {message.sender === 'bot' ? (
                      <BotMessageFormatter text={message.text} />
                    ) : (
                      message.text
                    )}
                  </div>
